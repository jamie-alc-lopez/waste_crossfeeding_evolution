...
% for later plotting. It uses numerical simulation results
%
...
for i = 1:length(Keq_val)
    ...
    sim_file = ['fig_data/no_osm_sweep_Keq_',num2str(Keq),...
        '_no_sat_simulation_results.mat'];
...
diagram_type = 'numerical'; %'numerical' or 'analytical'
...
Fig2ax = tight_subplot(2,2,gap,marg_h,marg_w);
...
load('fig_data/no_osm_sweep_Keq_1_no_sat_simulation_results.mat');
...
set(gca,'YScale','log')
hold on
dashline(D_vec,c1_vec,dmat(1,1),dmat(1,2),dmat(1,3),dmat(1,4),'Color',ext_c_color)
dashline(D_vec,c2_vec,dmat(2,1),dmat(2,2),dmat(2,3),dmat(2,4),'Color',ext_c_color)
dashline(D_vec,c3_vec,dmat(3,1),dmat(3,2),dmat(3,3),dmat(3,4),'Color',ext_c_color)
...
print(gcf, '-dpng','figs/fig2.png','-r800','-painters');
